...
public class StreamableMediaConverter extends BaseVideoConverter implements
    ...
    public BlobHolder convert(BlobHolder blobHolder,
        ...
        if (TransactionHelper.isTransactionActive()) {
            log.debug("Close transaction during streaming media, because compute can be long");
        ...
        try {
            ...
            totalTimeExecution = resultPivot.getExecTime();
            log.info("Command line for ogv conversion time execution : "
            ...
            totalTimeExecution = totalTimeExecution + resultMp4.getExecTime();
            log.info("Command line for mp4 conversion time execution: "
            ...
            totalTimeExecution = totalTimeExecution + resultHint.getExecTime();
            log.info("Command line for hint time execution: "
                    + resultHint.getExecTime());
            if (log.isDebugEnabled()) {
                log.debug(resultHint.getOutput());
            }

            log.info("Total time execution: " + totalTimeExecution);
            ...
            if (transactionWasActive) {
                log.debug("Start a new transaction after computing");
