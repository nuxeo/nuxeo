...
public class SQLSecurityManager implements SecurityManager {
    ...
    public void setACP(Document doc, ACP acp, boolean overwrite)
        ...
        if (overwrite) {
            ...
            // TODO: udpate ACP was not completely tested - may not work
            // correctly
            updateACP(doc, acp);
        }
    }

    public static void replaceACP(Document doc, ACP acp)
            throws SecurityException {
        if (acp == null) {
            removeACP(doc);
        } else {
    ...
    public static void updateACP(Document doc, ACP acp)
            throws SecurityException {
        if (acp == null) {
            return;
        }
    ...
    private static void collectOwners(ACP acp, Node acpNode)
            throws SecurityException {
        try {
            Value[] values = acpNode.getProperty(
                    NodeConstants.ECM_OWNERS.rawname).getValues();
    ...
    private static void collectACLs(ACP acp, Node acpNode)
            throws SecurityException {
        try {
    ...
    private static void collectACEs(ACL acl, Node aclNode)
            throws SecurityException {
        try {
    ...
    private static ACE getACE(Node aceNode) throws SecurityException {
        try {
            ...
            String principal = aceNode.getProperty(
                    NodeConstants.ECM_PRINCIPAL.rawname).getString();
            String permission = aceNode.getProperty(
                    NodeConstants.ECM_PERMISSION.rawname).getString();
    ...
    private static void writeACP(Node acpNode, ACP acp)
            throws SecurityException {
        String[] owners = acp.getOwners();
        writeOwners(acpNode, owners);
    ...
    private static void writeOwners(Node acpNode, String[] owners)
            throws SecurityException {
        try {
    ...
    private static void writeACL(Node acpNode, ACL acl)
            throws SecurityException {
        try {
            ...
            for (ACE ace : acl) {
                ...
                // then ignore this entry since it will be masked by the
                // existing one
                try {
                    Node aceNode = aclNode.addNode(name,
                            NodeConstants.ECM_NT_ACE.rawname);
                    aceNode.setProperty(NodeConstants.ECM_PRINCIPAL.rawname,
                            ace.getUsername());
                    aceNode.setProperty(NodeConstants.ECM_PERMISSION.rawname,
                            ace.getPermission());
                    aceNode.setProperty(NodeConstants.ECM_TYPE.rawname,
                            ace.isGranted());
    ...
    private static void updateACL(Node aclNode, ACL acl)
            throws SecurityException {
        try {
            // write ACEs
            for (ACE ace : acl) {
                ...
                if (aclNode.hasNode(name)) {
                    aclNode.getNodeByPath(name).remove();
                }
                Node aceNode = aclNode.addNode(name,
                        NodeConstants.ECM_NT_ACE.rawname);
                aceNode.setProperty(NodeConstants.ECM_PRINCIPAL.rawname,
    ...
    private static Node getACPNode(Node docNode, boolean create)
        ...
        try {
            ...
            if (docNode.hasNode(name)) {
                return docNode.getNodeByPath(NodeConstants.ECM_ACP.rawname);
            } else if (create) {
    ...
    public static ACL getACL(Document doc, String name)
    ...
    private static Node getACLNode(Node acpNode, String acl, boolean create)
        ...
        try {
            if (acpNode.hasNode(acl)) {
                return acpNode.getNodeByPath(acl);
            } else if (create) {
                return acpNode.addNode(acl, NodeConstants.ECM_NT_ACL.rawname);
            } else {
                return null;
            }
        } catch (RepositoryException e) {
    ...
    public static boolean hasACL(Document doc, String acl)
            throws DocumentException {
        Node docNode = ((JCRDocument) doc).getNode();
        Node acpNode = getACPNode(docNode, false);
        if (acpNode == null) {
    ...
    public static void removeACL(Document doc, String name)
            throws DocumentException {
        Node docNode = ((JCRDocument) doc).getNode();
        Node acpNode = getACPNode(docNode, false);
        if (acpNode == null) {
...
// } catch (DocumentException e) {
// //log.error(
    // "Exception appeared when traversing the object tree. Stopping the navigation."
    // );
// }
//
