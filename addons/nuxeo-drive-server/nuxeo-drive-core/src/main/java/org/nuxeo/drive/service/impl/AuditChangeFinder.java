...
public class AuditChangeFinder implements FileSystemChangeFinder {
    ...
    public List<FileSystemItemChange> getFileSystemChanges(CoreSession session, Set<IdRef> lastActiveRootRefs,
        ...
        return getFileSystemChanges(session, lastActiveRootRefs, activeRoots, lastSuccessfulSyncDate, syncDate, false,
                limit);
    }

    @Override
    public List<FileSystemItemChange> getFileSystemChangesIntegerBounds(CoreSession session,
            Set<IdRef> lastActiveRootRefs, SynchronizationRoots activeRoots, long lowerBound, long upperBound, int limit)
            throws ClientException, TooManyChangesException {
        return getFileSystemChanges(session, lastActiveRootRefs, activeRoots, lowerBound, upperBound, true, limit);
    }

    protected List<FileSystemItemChange> getFileSystemChanges(CoreSession session, Set<IdRef> lastActiveRootRefs,
            SynchronizationRoots activeRoots, long lowerBound, long upperBound, boolean integerBounds, int limit)
            throws ClientException, TooManyChangesException {
        ...
        // roots
        List<LogEntry> entries = queryAuditEntries(session, activeRoots, lowerBound, upperBound, integerBounds, limit);
        ...
        for (LogEntry entry : entries) {
            if (NuxeoDriveEvents.EVENT_CATEGORY.equals(entry.getCategory())) {
                ...
                entries = queryAuditEntries(session, updatedActiveRoots, lowerBound, upperBound, integerBounds, limit);
                break;
            }
        }
    ...
    @SuppressWarnings("unchecked")
    protected List<LogEntry> queryAuditEntries(CoreSession session, SynchronizationRoots activeRoots, long lowerBound,
        ...
        if (!activeRoots.getPaths().isEmpty()) {
            ...
            auditQuerySb.append(" and (log.eventId = 'documentCreated' or log.eventId = 'documentModified' or log.eventId = 'documentMoved' or log.eventId = 'documentCreatedByCopy' or log.eventId = 'documentRestored' or log.eventId = 'documentLocked' or log.eventId = 'documentUnlocked')");
        ...
        for (LogEntry entry : entries) {
            ...
            if (log.isDebugEnabled()) {
                log.debug(String.format("Change with eventId=%d detected at eventDate=%s, logDate=%s: %s on %s",
                        entry.getId(), entry.getEventDate(), entry.getLogDate(), entry.getEventId(), entry.getDocPath()));
            }
    ...
    protected FileSystemItemChange getFileSystemItemChange(CoreSession session, DocumentRef docRef, LogEntry entry,
        ...
        log.debug(String.format(
                "Document %s is adaptable as a FileSystemItem, providing it to the FileSystemItemChange entry.", docRef));
