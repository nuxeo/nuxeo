...
public class SuggesterGroupDescriptor implements Cloneable {
    ...
    public void mergeFrom(SuggesterGroupDescriptor newDescriptor)
        ...
        for (SuggesterGroupItemDescriptor newSuggesterGroupItem : newDescriptor.getSuggesters()) {
            ...
            // manage appendBefore, appendAfter or no particular attributes
                ...
                // manage appendBefore
                if (appendBeforeSuggesterName != null) {
                    boolean isSuggesterAppended = appendBefore(
                            appendBeforeSuggesterName, newSuggesterName);
                ...
                // manage appendAfter
                    ...
                    boolean isSuggesterAppended = appendAfter(
                            appendAfterSuggesterName, newSuggesterName);
                    if (!isSuggesterAppended) {
                        logExistingSuggesterName(newSuggesterName);
                    }
                }
                ...
                else if (appendBeforeSuggesterName == null
                        && appendAfterSuggesterName == null) {
                    boolean isSuggesterAppended = appendAfter(null,
                            newSuggesterName);
                    if (!isSuggesterAppended) {
                        logExistingSuggesterName(newSuggesterName);
                    }
    ...
    /**
     * Removes the suggester named {@code suggesterName} from the
     * {@code #suggesters} list.
     *
        ...
        while (suggestersIt.hasNext()) {
            ...
            if (suggesterName.equals(suggesterGroupItem.getName())) {
                ...
                log.debug(String.format(
                        "Removed suggester '%s' from suggesterGroup '%s'.",
                        suggesterName, name));
                return true;
            }
        }
     ...
     * {@code suggesterName} in the {@code #suggesters} list, or -1 if
     * {@code suggesterName} is null or if this list does not contain the
     * element.
     *
     * @param suggesterName the suggester name
     * @return the index of the first occurrence of the element named
     *         {@code suggesterName} in the {@code #suggesters} list, or -1 if
     *         {@code suggesterName} is null or if this list does not contain
     *         the element
     */
    ...
    /**
     * Unless a suggester named {@code newSuggesterName} already exists in the
     * {@code #suggesters} list, appends a new
     * {@code SuggesterGroupItemDescriptor} named {@code newSuggesterName} just
     * before the suggester named {@code suggesterName} in the
     * {@code #suggesters} list. If the suggester named {@code suggesterName}
     * does not exist, appends the new suggester at the beginning of the list.
     *
     * @param suggesterName the suggester name
     * @param newSuggesterName the name of the suggester to append
     * @return true, if the suggester named {@code newSuggesterName} was
     *         appended to the {@code #suggesters} list
     */
     ...
     * {@code #suggesters} list, appends a new
     * {@code SuggesterGroupItemDescriptor} named {@code newSuggesterName} just
     * after the suggester named {@code suggesterName} in the
     * {@code #suggesters} list. If the suggester named {@code suggesterName}
     * does not exist, appends the new suggester at the end of the list.
     *
     * @param suggesterName the suggester name
     * @param newSuggesterName the name of the suggester to append
     * @return true, if the suggester named {@code newSuggesterName} was
     ...
     * {@code #suggesters} list, appends a new
     * {@code SuggesterGroupItemDescriptor} named {@code newSuggesterName} just
     * before (if {@code before} is true) or after the suggester named
     * {@code suggesterName} in the {@code #suggesters} list. If the suggester
     * named {@code suggesterName} does not exist, appends the new suggester at
     * the beginning or the end of the list, depending on {@code before}.
     *
     * @param suggesterName the suggester name
     * @param newSuggesterName the name of the suggester to append
     * @return true, if the suggester named {@code newSuggesterName} was
     *         appended to the {@code #suggesters} list
     */
    protected boolean append(String suggesterName, String newSuggesterName,
            boolean before) {
        ...
        // new suggester
        SuggesterGroupItemDescriptor newSuggester = new SuggesterGroupItemDescriptor(
                newSuggesterName);
        int indexOfSuggester = indexOf(suggesterName);
        if (indexOfSuggester > -1) {
            // suggester found, append new suggester before or after it
            int indexOfNewSuggester = before ? indexOfSuggester
                    : indexOfSuggester + 1;
            suggesters.add(indexOfNewSuggester, newSuggester);
            log.debug(String.format(
                    "Appended suggester '%s' %s suggester '%s' in suggesterGroup '%s'.",
                    newSuggesterName, before ? "before" : "after",
                    suggesterName, name));
        } else {
            ...
            if (before) {
                ...
                if (suggesterName != null) {
                    log.warn(String.format(
                            ...
                            newSuggesterName, suggesterName, name,
                            suggesterName));
                }
            } else {
    ...
    /**
     * Logs that the suggester named {@code newSuggesterName} already exists in
     * the {@code #suggesters} list and therefore won't be appended to it.
     *
