        ...
        try {
            ...
            if (blob.getFilename().endsWith("mp4")) {
                log.debug(blob.getFilename()
                        + " is already a mp4 file, don't try to convert it");
                ...
                ExecResult resultMp4 = cleService.execCommand(
                        HANDBRAKE_CONVERT_MP4, paramsForStreamable);
                if (!resultMp4.isSuccessful()) {
                    throw new ConversionException(
                            "Failed to build mp4 version of "
                                    + blob.getFilename()
                                    + ": "
            ...
            paramsForHint.addNamedParameter("filePath",
                    outFile.getAbsolutePath());
            ExecResult resultHint = cleService.execCommand(MP4BOX_HINT_MEDIA,
                    paramsForHint);
            ...
            log.info(String.format("mp4 hinting of '%s' execution time: %ds",
