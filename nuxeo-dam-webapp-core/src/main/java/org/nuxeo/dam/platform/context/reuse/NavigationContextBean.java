...
public class NavigationContextBean implements NavigationContextLocal,
    ...
    // valid: like Ref
    // NXGED-1021: we need to re-inject this from the context with the values
    // that might
    // be set from a create page. If we don't do this the document filled in
    // with form
    // data will be lost in the context replaced later with an empty one that is
    // de-seriliazed
    // with this bean.
    ...
    public RepositoryLocation getCurrentServerLocation() {
         ...
         * log.warn("currentServerLocation is null : is this normal"); if
         * (currentDocument!=null) { String repoName =
         * currentDocument.getRepositoryName();
         * log.warn("trying to automatically set currentServerLocation is null"
         * ); try { setCurrentServerLocation(new RepositoryLocation(repoName));
         * } catch (ClientException e) {
     ...
     * 
     * @see
     * org.nuxeo.ecm.webapp.context.NavigationContext#navigateToDocument(org
     * .nuxeo.ecm.core.api.DocumentModel, org.nuxeo.ecm.core.api.VersionModel)
     */
