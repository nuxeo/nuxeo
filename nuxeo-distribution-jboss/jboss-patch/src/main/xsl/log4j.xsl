<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <!--
    remove previous declarations (if there) so that they don't end up twice in
    the file
  -->
  ...
  <xsl:template
    match="category[@name='org.jboss.ejb3.cache.simple.StatefulSessionFilePersistenceManager']">
  </xsl:template>
  ...
  <xsl:template match="category[@name='org.jboss.system.server.Server']">
  </xsl:template>
  ...
  <xsl:template
    match="category[@name='org.hibernate.engine.StatefulPersistenceContext.ProxyWarnLog']">
  </xsl:template>
  <xsl:template
    match="category[@name='org.hibernate.impl.SessionFactoryObjectFactory']">
  </xsl:template>
  ...
  <xsl:template
    match="category[@name='org.compass.core.lucene.engine.optimizer.ScheduledLuceneSearchEngineOptimizer']">
  </xsl:template>
  <xsl:template
    match="category[@name='org.apache.jackrabbit.core.query.lucene.IndexMerger']">
  </xsl:template>
  ...
  <xsl:template match="category[@name='org.nuxeo.osgi.application.loader.FrameworkLoader']">
  </xsl:template>
  <xsl:template match="category[@name='org.nuxeo.osgi.application.StandaloneApplication']">
  </xsl:template>
  <xsl:template match="category[@name='org.nuxeo.osgi.application.FrameworkBootstrap']">
  </xsl:template>
  <xsl:template match="category[@name='org.nuxeo.runtime.jboss.deployment.NuxeoDeployer']">
  </xsl:template>
  <xsl:template
    match="category[@name='org.apache.myfaces.renderkit.html.util.DefaultAddResource']">
  </xsl:template>
  <xsl:template
    match="category[@name='javax.enterprise.resource.webcontainer.jsf.renderkit']">
  </xsl:template>
  <xsl:template
    match="category[@name='javax.enterprise.resource.webcontainer.jsf.application']">
  </xsl:template>
  <xsl:template
    match="category[@name='javax.enterprise.resource.webcontainer.jsf.lifecycle']">
  </xsl:template>
  <xsl:template
    match="category[@name='javax.enterprise.resource.webcontainer.jsf']">
  </xsl:template>
  ...
  <!-- reinsert before org.apache category -->
  ...
  <xsl:template match="appender[@name='CONSOLE']">
