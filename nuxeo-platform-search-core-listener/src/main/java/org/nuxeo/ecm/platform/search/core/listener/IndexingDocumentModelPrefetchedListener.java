...
public class IndexingDocumentModelPrefetchedListener extends
    ...
    public void notifyEvent(CoreEvent event) throws Exception {
        ...
        Object blockIndexing = null;
        Map<String, ?> eventInfo = event.getInfo();
        if (eventInfo != null) {
            blockIndexing = eventInfo.get(EventMessage.BLOCK_SYNC_INDEXING);
        }
    ...
    protected void indexDocumentModelPrefetch(DocumentModel dm)
        ...
        String docTitle;
        try {
            docTitle = (String) dm.getProperty("dublincore", "title");
        ...
        if (docTitle == null || docTitle.length() == 0) {
            ...
            // ==> do not index it
            // FIXME AT: this is a hack, this event should not be caught by this
            // listener.
            log.debug("empty create call");
            return;
        }

        if (dm.getContextData(EventMessage.PERFORM_FULL_SYNC_INDEXING) != null
                && (Boolean) dm.getContextData(EventMessage.PERFORM_FULL_SYNC_INDEXING)) {
            log.debug("Indexing all for doc@" + dm.getPathAsString());
            service.index(computeResourcesFor(dm), true);
        } else {
