...
namespace NuxeoCtl
{
	partial class MainForm
	{
		...
		private void InitializeComponent()
		{
            ...
            this.nxCtlState = new System.Windows.Forms.LinkLabel();
            ...
            this.nxCtlState.Location = new System.Drawing.Point(37, 189);
            ...
            this.nxCtlState.LinkClicked += new System.Windows.Forms.LinkLabelLinkClickedEventHandler(this.nxCtlState_LinkClicked);
            // 
            // MainForm
            // 
        ...
        private System.Windows.Forms.LinkLabel nxCtlState;
        ...
        private void DisplayCtlStateText(String text, String url)
        {
            if (nxCtlState.InvokeRequired)
            {
                nxCtlState.Invoke(new EventHandler(delegate {
                    ChangeText(text, url);
                }));
            }
            else
		...
		private void DisplayCtlStateText(String text) {
            DisplayCtlStateText(text, "");
		}

        private void ChangeText(String text, String url) {
            nxCtlState.Text = text;

            if (String.IsNullOrEmpty(url))
            {
                nxCtlState.Links.Clear();
            }
            else
            {
                nxCtlState.Text += " " + url;

                int startOffset = nxCtlState.Text.Length - url.Length;
                nxCtlState.Links.Add(startOffset, url.Length, url);
            }
        }
		...
		bool StartApplication() {
			...
			if (!nxControl.Initialized || nxControl.Configure() == false)
			{
		...
		void nxControl_ProcessStarted(object sender, EventArgs e)
		{
            ...
            DisplayCtlStateText("Nuxeo is now accessible at ", String.Format("http://{0}:8080/nuxeo/", address));
		}
