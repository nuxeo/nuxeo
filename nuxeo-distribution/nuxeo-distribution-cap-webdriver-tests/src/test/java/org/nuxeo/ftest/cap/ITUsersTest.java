...
public class ITUsersTest extends AbstractTest {
    ...
    public void userCanChangeItsOwnPassword() throws Exception {
        ...
        OwnUserChangePasswordFormPage passPage = profilePage.getChangePasswordUserTab().changePassword(newPassword,
                "newpwd_not_matching");
        assertEquals(passPage.getErrorFeedbackMessage(), "Please correct errors.");
        Locator.waitForTextPresent(passPage.getForm(), "Passwords do not match.");

        passPage.changePassword("newpwd");
        logout();
