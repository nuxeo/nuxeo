...
case "$ASSEMBLY" in
  ...
  ;;
    nuxeo-core)
        ...
        mvn -Dnuxeo.ear.assembly=$ASSEMBLY $MVN_OPTS package $@
  ;;
    nuxeo-indexing)
  ...
  ;;
    nuxeo-simplewebapp)
        echo "building complete ear without unnecessary facades"
        mvn -Dnuxeo.ear.assembly=$ASSEMBLY $MVN_OPTS package $@
  ;;
    nuxeo-3parts)
        echo "building 3 JVMs nuxeo distrib"
        mvn -Dnuxeo.ear.assembly=nuxeo-core $MVN_OPTS install package $@
        mvn -Dnuxeo.ear.assembly=nuxeo-indexing $MVN_OPTS package -N $@
        mvn -Dnuxeo.ear.assembly=nuxeo-webplatform $MVN_OPTS package -N $@
  ;;
    old-nuxeo-3parts)
        echo "building 3 JVMs nuxeo distrib"
        mvn -Dnuxeo.ear.assembly=old-nuxeo-core $MVN_OPTS install package $@
  ...
  ;;
    old-nuxeo-2parts)
        echo "building 2 JVMs nuxeo distrib"
        mvn -Dnuxeo.ear.assembly=old-nuxeo-platform-stateful $MVN_OPTS install package $@
  ...
  ;;
    ...
    help)
        ...
        echo "nuxeo-simplewebapp : building complete ear without unnecessary facades"
