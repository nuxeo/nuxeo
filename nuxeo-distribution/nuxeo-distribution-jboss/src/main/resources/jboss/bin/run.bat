...
rem Fix JRE 1.5/1.6 issue with script-api.jar
"%JAVA%" -version 2>&1 | findstr /I 1.6 > nul
if not errorlevel == 1 (set IS_JRE_6=true)
set NUXEO_EAR="%JBOSS_HOME%\server\default\deploy\nuxeo.ear"
if x%IS_JRE_6% == xtrue (
  echo "Java 1.6 detected"
  if exist %NUXEO_EAR%\lib\script-api-1.0.jar (
    echo "Moving scripting API out of classpath (already provided by Java6)"
    mkdir "%NUXEO_EAR%\lib-jre1.5" > nul
    move "%NUXEO_EAR%\lib\script-api-1.0.jar" "%NUXEO_EAR%\lib-jre1.5\"
  )
) else (
  echo "Assumed Java 1.5"
  if not exist %NUXEO_EAR%\lib\script-api-1.0.jar (
    echo "Moving scripting API into classpath"
    move "%NUXEO_EAR%\lib-jre1.5\script-api-1.0.jar" "%NUXEO_EAR%\lib\"
    if errorlevel 1 (echo "Missing scripting API in %NUXEO_EAR%\lib\")
  )
)
