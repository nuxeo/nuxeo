...
TinyMCE_Engine.prototype = {
	init : function(settings) {
		...
		this.callbacks = new Array('onInit', 'getInfo', 'getEditorTemplate',
		...
		if (tinyMCE.getParam("editor_css", false)) {
			...
			if (this.settings.editor_css != '')
				this.settings['editor_css'] = tinyMCE.baseURL + "/themes/"
						+ theme + "/css/editor_ui.css"
		}
			...
			if (ar[x] != null && ar[x] != 'null' && ar[x].length > 0) {
				...
				if (!skip) {
					if (tinyMCE.settings.strict_loading_mode) {
						...
						document
								.write('<link href="' + ar[x] + '" rel="stylesheet" type="text/css" />');
	...
	setupContent : function(editor_id) {
		...
		var parentElm = inst.targetDoc
				.getElementById(inst.editorId + '_parent');
	...
	handleEvent : function(e) {
		...
		case "submit":
			tinyMCE
					.removeTinyMCEFormElements(tinyMCE.isMSIE ? window.event.srcElement
							: e.target);
			tinyMCE.triggerSave();
		...
		case "keydown":
			...
			if (tinyMCE.isGecko && e.type == "keyup" && e.keyCode == 9)
				tinyMCE.handleVisualAid(tinyMCE.selectedInstance.getBody(),
		...
		for ( var c = 0; c < tinyMCE.configs.length; c++) {
			...
			for ( var i = 0; i < elementRefAr.length; i++) {
				...
				if (tinyMCE.settings['ask']
						|| tinyMCE.settings['convert_on_click']) {
					if (tinyMCE.isGecko) {
	...
	_customCleanup : function(inst, type, content) {
		...
		if (customCleanup != ""
				&& eval("typeof(" + customCleanup + ")") != "undefined")
	...
	importPluginLanguagePack : function(name) {
		...
		tinyMCE
				.loadScript(b + '/langs/' + tinyMCE.settings['language']
						+ '.js')
	},
	...
	openWindow : function(template, args) {
		...
		if (html) {
			...
			var win = window.open("", "mcePopup" + new Date().getTime(), "top="
					+ y + ",left=" + x + ",scrollbars=" + scrollbars
			...
			if ((tinyMCE.isRealIE) && resizable != 'yes'
				...
				height += 10;
				var features = "resizable:" + resizable + ";scroll:"
						+ scrollbars
		...
		case "TABLE":
			...
			for ( var y = 0; y < el.rows.length; y++) {
				for ( var x = 0; x < el.rows[y].cells.length; x++) {
					var cn = tinyMCE.getVisualAidClass(tinyMCE.getAttrib(
						...
						if (selectorText) {
							...
							for ( var c = 0; c < rules.length; c++) {
								...
								if (rule
									...
									continue;
								if (rule.indexOf('.') != -1) {
									var cssClass = rule.substring(rule
											.indexOf('.') + 1);
									var addClass = true;
									for ( var p = 0; p < output.length
											&& addClass; p++) {
	...
	xmlEncode : function(s, skip_apos) {
		return s ? ('' + s).replace(!skip_apos ? this.xmlEncodeAposRe
				: this.xmlEncodeRe, function(c, b) {
			switch (c) {
			case '&':
				return '&amp;';
			case '"':
...
TinyMCE_Control.prototype = {
	...
	execCommand : function(command, user_interface, value) {
		var doc = this.getDoc(), win = this.getWin(), focusElm = this
				.getFocusElement();
		if (!new RegExp(
		...
		case "SetStyleInfo":
			...
			if (tinyMCE.isIE) {
				...
				if (sel.isCollapsed
						|| (new RegExp('td|tr|tbody|table', 'gi')
	...
	_onAdd : function(replace_element, form_element_name, target_document) {
		...
		var deltaWidth = editorTemplate['delta_width'] ? editorTemplate['delta_width']
				: 0;
		var deltaHeight = editorTemplate['delta_height'] ? editorTemplate['delta_height']
				: 0;
		var html = '<span id="' + this.editorId
				+ '_parent" class="mceEditorContainer">'
				+ editorTemplate['html'];
		...
		if (dynamicIFrame) {
			var html = tinyMCE.getParam('doctype')
	...
	triggerSave : function(skip_cleanup, skip_callback) {
		...
		var htm = skip_cleanup ? this.getBody().innerHTML : tinyMCE
				._cleanupHTML(this, this.getDoc(), this.settings, this
						.getBody(), tinyMCE.visualAid, true, true);
...
TinyMCE_Engine.prototype.cleanupHTMLCode = function(s) {
	...
	s = s.replace(new RegExp('<(img|br|hr)([^>]*)><\\/(img|br|hr)>', 'gi'),
			'<$1$2 />');
	if (tinyMCE.isIE)
...
TinyMCE_Engine.prototype._cleanupHTML = function(inst, doc, config, elm,
		visual, on_save, on_submit, inn) {
	...
	if (on_save) {
		h = h.replace(new RegExp(
				' ?(mceItem[a-zA-Z0-9]*|' + s.visual_table_class + ')', 'g'),
				'');
...
TinyMCE_Cleanup.prototype = {
	...
	parseRuleStr : function(s) {
		var ta, p, r, a, i, x, px, t, tn, y, av, or = tinyMCE
				.clearArray(new Array()), dv;
	...
	xmlEncode : function(s, skip_apos) {
		var cl = this, re = !skip_apos ? this.xmlEncodeAposRe
				: this.xmlEncodeRe;
		this._setupEntities();
...
TinyMCE_Engine.prototype.getElementByAttributeValue = function(n, e, a, v) {
	return (n = this.getElementsByAttributeValue(n, e, a, v)).length == 0 ? null
			: n[0]
};
...
TinyMCE_Engine.prototype.convertRelativeToAbsoluteURL = function(base_url,
		relative_url) {
	...
	var absPath = (len <= 0 ? "" : "/") + baseURLParts.slice(0, len).join('/')
			+ "/" + relURLParts.join('/');
	var start = "", end = "";
	...
	if (on_save && tinyMCE.getParam('relative_urls')) {
		...
		if (urlParts['host'] == tmpUrlParts['host']
			...
			return tinyMCE.convertAbsoluteURLToRelativeURL(
					tinyMCE.settings['document_base_url'], curl)
	}
...
TinyMCE_Engine.prototype._setEventsEnabled = function(node, state) {
	...
	var events = [ 'onfocus', 'onblur', 'onclick', 'ondblclick', 'onmousedown',
			'onmouseup', 'onmouseover', 'onmousemove', 'onmouseout',
			'onkeypress', 'onkeydown', 'onkeydown', 'onkeyup' ];
...
TinyMCE_Engine.prototype.setEventHandlers = function(inst, s) {
	var doc = inst.getDoc(), ie, ot, i, f = s ? tinyMCE.addEvent
			: tinyMCE.removeEvent;
	ie = [ 'keypress', 'keyup', 'keydown', 'click', 'mouseup', 'mousedown',
			'controlselect', 'dblclick' ];
	ot = [ 'keypress', 'keyup', 'keydown', 'click', 'mouseup', 'mousedown',
			'focus', 'blur', 'dragdrop' ];
	inst.switchSettings();
	if (tinyMCE.isIE) {
...
var TinyMCE_ForceParagraphs = {
	_insertPara : function(inst, e) {
		...
		if (startBlock
				&& new RegExp('absolute|relative|static', 'gi')
		...
		if (startBlock != null) {
			...
			if (blockName == "TD"
					|| blockName == "TABLE"
					|| (blockName == "DIV" && new RegExp('left|right', 'gi')
