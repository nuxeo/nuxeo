...
public class LogEntryCallbackListener {
    ...
    private static final Log log = LogFactory.getLog(LogEntryCallbackListener.class);

    @PostPersist
    public void logCreation(LogEntry logEntry) {
        if (log.isDebugEnabled()) {
            log.debug(String.format(
                    "LogEntryBean with id '%s' after event '%s' for document '%s' (ref: '%s')",
