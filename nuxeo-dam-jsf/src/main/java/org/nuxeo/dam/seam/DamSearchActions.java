...
public class DamSearchActions implements Serializable {
    ...
    public String getAssetPermanentLinkUrl() throws ClientException, UnsupportedEncodingException {
        ...
        DocumentModel damCurrentDocument = mainTabsActions.getDocumentFor(DAM_MAIN_TAB_ACTION);
        DocumentView docView = new DocumentViewImpl(new DocumentLocationImpl(documentManager.getRepositoryName(),
                new IdRef(damCurrentDocument.getId())));
        docView.setViewId("asset");
        DocumentViewCodecManager documentViewCodecManager = Framework.getLocalService(DocumentViewCodecManager.class);
        return documentViewCodecManager.getUrlFromDocumentView(DAM_ID_CODEC, docView, true, BaseURL.getBaseURL());
    }
