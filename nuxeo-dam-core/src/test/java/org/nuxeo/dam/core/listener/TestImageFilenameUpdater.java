...
public class TestImageFilenameUpdater extends SQLRepositoryJUnit4 {
	...
	public void testListener() throws Exception {
		...
		DocumentModel picture = session.createDocumentModel(importSet
				.getPathAsString(), "pictureTest", "Picture");
		...
		picture
				.setPropertyValue("dc:description",
		...
		PictureResourceAdapter adapter = picture
				.getAdapter(PictureResourceAdapter.class);
		assertNotNull(adapter);
		Blob blob = new FileBlob(new File(this.getClass().getClassLoader()
				.getResource("big_nuxeo_logo.jpg").toURI()));
		assertNotNull(blob);
		...
		picture = session.getChildren(importSet.getRef()).get(0);
		Map<String, Object> pictureMap = picture.getDataModel(PICTURE_SCHEMA)
				.getMap();
		...
		picture = session.getChildren(importSet.getRef()).get(0);
		viewsList = (List<Map<String, Object>>) picture
		...
		for (Map<String, Object> el : viewsList) {
			...
			assertTrue((title + "_" + "big_nuxeo_logo edit.jpg")
					.equals(fileBlob.getFilename()));
		}
	}
