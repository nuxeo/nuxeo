function getBaseURLObject(link) {
	var baseURLObject = new Array();
	// set source
	baseURLObject[0] = link;

	var index1 = -1, index2 = -1, temp = -1;

	index1 = link.indexOf("://");
	// set protocol
	baseURLObject[1] = link.substring(0, index1);
	if (baseURLObject[1] == "file")
	...
	// set authority
	baseURLObject[2] = link.substring(index1, index2);
	index1 = baseURLObject[2].indexOf('@');

	if (index1 != -1) {
		// set userInfo
		baseURLObject[3] = baseURLObject[2].substring(0, index1);
		temp = baseURLObject[3].indexOf(':');
		if (temp != -1) {
			// set user
			baseURLObject[4] = baseURLObject[3].substring(0, temp);
			// set password
			baseURLObject[5] = baseURLObject[3].substring(temp + 1,
					baseURLObject[3].length);
		}

		// temp set host
		baseURLObject[6] = baseURLObject[2].substring(index1 + 1,
				baseURLObject[2].length);
	} else {
		// temp set host
		baseURLObject[6] = baseURLObject[2];
	}

	temp = baseURLObject[6].indexOf(':'); // Has a port ?
	if (temp != -1) {
		// set port first
		baseURLObject[7] = baseURLObject[6].substring(temp + 1,
				baseURLObject[6].length);
		// final set host
		baseURLObject[6] = baseURLObject[6].substring(0, temp);
	}

	// set relative
	baseURLObject[8] = link.substring(index2);

	// set temp path
	baseURLObject[9] = baseURLObject[8];
	index1 = baseURLObject[9].indexOf('?');
	if (index1 != -1) {
		// set temp path
		baseURLObject[9] = baseURLObject[9].substring(0, index1);
	}
	...
	index2 = baseURLObject[9].lastIndexOf("/");
	if (index2 != -1) {
		// set directory
		baseURLObject[10] = baseURLObject[9].substring(0, index2);
		// set file
		baseURLObject[11] = baseURLObject[9].substring(index2 + 1,
				baseURLObject[9].length);
	} else {
		// set directory
		baseURLObject[10] = baseURLObject[9];
	}

	index1 = baseURLObject[8].indexOf('?');
	if (index1 != -1) {
		// set query temp
		baseURLObject[12] = baseURLObject[8].substring(index1 + 1,
				baseURLObject[8].length);
	}

	if (baseURLObject[12] == null)
		return baseURLObject;

	index1 = baseURLObject[12].indexOf('#');
	if (index1 != -1) {
		// setting anchor if any
		baseURLObject[13] = baseURLObject[12].substring(index1 + 1,
				baseURLObject[12].length);
		// setting final query
		baseURLObject[12] = baseURLObject[12].substring(0, index1);
	}
	...
	tinymce.create("tinymce.util.URI", {
		URI : function(e, g) {
			...
			if (e.indexOf("/") === 0 && e.indexOf("//") !== 0) {
				e = (g.base_uri ? g.base_uri.protocol || "http" : "http")
						+ "://mce_host" + e
			}
			if (e.indexOf(":/") === -1 && e.indexOf("//") !== 0) {
				e = (g.base_uri.protocol || "http") + "://mce_host"
						+ f.toAbsPath(g.base_uri.path, e)
			}
			e = e.replace(/@@/g, "(mce_at)");
			e = getBaseURLObject(e);
			a( [ "source", "protocol", "authority", "userInfo", "user",
					"password", "host", "port", "relative", "path",
					"directory", "file", "query", "anchor" ], function(b, j) {
				var k = e[j];
				if (k) {
		...
		toRelative : function(b) {
			...
			if ((b.host != "mce_host" && c.host != b.host && b.host)
					|| c.port != b.port || c.protocol != b.protocol) {
				return b.getURI()
			}
		...
		toAbsPath : function(e, f) {
			...
			return "/" + e.slice(0, c).join("/") + "/" + g.reverse().join("/")
					+ d
		},
		getURI : function(d) {
			var c, b = this;
