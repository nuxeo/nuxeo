...
public abstract class AbstractWebType implements WebType {
    ...
    protected Class<? extends WebObject> resolveObjectClass(WebTypeManager mgr, String className) throws WebException {
        ...
        if (className == null) {
            //TODO: use a package path configured in WebEngine2
            className = "org.nuxeo.ecm.webengine.rest.model.impl.."+getName()+"Object";
            try {
                klass = (Class<? extends WebObject>)mgr.getEngine().getScripting().loadClass(className);
    ...
    public Action getAction(String action) throws NoSuchActionException {
