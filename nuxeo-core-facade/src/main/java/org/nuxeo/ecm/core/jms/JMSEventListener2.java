...
public class JMSEventListener2 extends AbstractEventListener implements
    ...
    private static final Log log = LogFactory.getLog(JMSEventListener2.class);
    ...
    public void onCommit(CoreEvent[] events) {
        ...
        try {
            for (OperationEvent event : filterEvents(events)) {
                publisher.publish(event, event.getId());
            }
        } catch (Exception e) {
