...
/**
 * Implementation of a {@link Document} proxy for Document-Based Storage.
 *
 * @since 5.9.4
 */
public class DBSDocumentProxy implements Document {

    /** The proxy seen as a normal doc. */
    protected final Document proxy;

    /** The target. */
    protected Document target;

    public DBSDocumentProxy(Document proxy, Document target) {
        this.proxy = proxy;
        this.target = target;
    }
    ...
    public boolean equals(Object other) {
        ...
        if (other instanceof DBSDocumentProxy) {
            return equals((DBSDocumentProxy) other);
        }
        return false;
    }

    private boolean equals(DBSDocumentProxy other) {
