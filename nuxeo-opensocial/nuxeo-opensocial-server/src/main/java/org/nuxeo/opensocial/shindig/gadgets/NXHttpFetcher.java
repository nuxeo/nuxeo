...
/**
 * This is basically a copy of BasicHttpFetcher
 * We don't inherit from it since there are private methos
 * that are not accessible from the fetch method.
 *
 * We do copy the class because we want to have proxy
 * authentication working since HttpClient doesnt use
 * the java authenticator mecanism.
 *
 * @author dmetzler
 *
 */
public class NXHttpFetcher implements HttpFetcher {
    ...
    public HttpResponse fetch(HttpRequest request) {
       ...
       */
      if("true".equals(Framework.getProperty(SHINDIG_PROXY_PROXY_SET))) {
          String proxyHost = Framework.getProperty(SHINDIG_PROXY_PROXY_HOST);
          int proxyPort = Integer.parseInt(Framework.getProperty(SHINDIG_PROXY_PROXY_PORT));
