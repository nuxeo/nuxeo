...
public class DashboardSpaceProvider extends SingleDocSpaceProvider {
    ...
    private static final Log log = LogFactory
            .getLog(DashboardSpaceProvider.class);
    ...
    protected static DocumentModel getOrCreateParentUnivers(CoreSession session)
        ...
        PathRef universePath = new PathRef(getUserPersonalWorkspace(session)
                .getPathAsString()
        ...
        if (session.exists(universePath)) {
            ...
            universeDoc = session.createDocumentModel(getUserPersonalWorkspace(
                    session).getPathAsString(), DASHBOARD_UNIVERSE_NAME,
                    "HiddenFolder");
            universeDoc.setProperty("dc", "title", "nuxeo dashboard universe");
            universeDoc.setProperty("dc", "description",
    ...
    protected static void createInitialGadgets(CoreSession session,
        ...
        LocaleSelector localeSelector = (LocaleSelector) Component
                .getInstance("org.jboss.seam.international.localeSelector");
        String language = "en";
        ...
        createGadgetForInitialDashboard(session, space, "tasks", title,
        ...
        title = TranslationHelper.getLabel(labelKey, language);
        createGadgetForInitialDashboard(session, space, "userworkspaces",
                title, COLS[1], new Integer(0));
