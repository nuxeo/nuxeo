...
public class PreferencesPresenter extends
    ...
    public PreferencesPresenter(final Display display, final EventBus eventBus,
        ...
        this.data = model.getWebContent(portletPresenter.getDisplay()
                .getId());
        this.model = model;
    ...
    private void fetchContent() {
        ...
        if (data instanceof OpenSocialData) {
            for (Entry<String, UserPref> userPref : ((OpenSocialData) data).getUserPrefs()
                    .entrySet()) {
    ...
    private void addUserPref(UserPref pref) {
        ...
        case STRING:
            //TODO Work only for PictureBook ! Has to be improved for any type of document
            if (pref.getName()
                    .startsWith("NXID_")) {
        ...
        case ENUM:
            if (pref.getName()
                ...
                for (Entry<String, String> enumValues : pref.getEnumValues()
                        .entrySet()) {
    ...
    private void registerTitleChangement() {
        registerHandler(display.getTitleEvent()
                .addKeyUpHandler(new KeyUpHandler() {
                    public void onKeyUp(KeyUpEvent arg0) {
                        portletPresenter.setTitle(display.getTitleBox()
                                .getText());
                    }
                }));
    }
    ...
    public void rollBack() {
        ...
        portletPresenter.setBorderColor(data.getPreferences()
                .get(DefaultPortletPreference.WC_BORDER_COLOR.toString()));

        portletPresenter.setTitleColor(data.getPreferences()
                .get(DefaultPortletPreference.WC_TITLE_COLOR.toString()));

        portletPresenter.setHeaderColor(data.getPreferences()
                .get(DefaultPortletPreference.WC_HEADER_COLOR.toString()));
    }
    ...
    private void registerPreferencesSave() {
        registerHandler(display.getSaveButton()
                .addClickHandler(new ClickHandler() {
                    ...
                    public void onClick(ClickEvent event) {
                        data.setTitle(display.getTitleBox()
                                .getText());

                        data.addPreference(
                                DefaultPortletPreference.WC_TITLE_COLOR.toString(),
                                ((ColorsPanelWidget) display.getTitleColors()).getSelectedColor()
                                        .getColorAsString());

                        data.addPreference(
                                DefaultPortletPreference.WC_BORDER_COLOR.toString(),
                                ((ColorsPanelWidget) display.getBorderColors()).getSelectedColor()
                                        .getColorAsString());

                        data.addPreference(
                                DefaultPortletPreference.WC_HEADER_COLOR.toString(),
                                ((ColorsPanelWidget) display.getHeaderColors()).getSelectedColor()
                                        .getColorAsString());
                        ...
                        if (data instanceof OpenSocialData) {
                            for (Widget widget : prefsValues) {
                                ...
                                String value = ((HasValue) widget).getValue()
                                        .toString();
                                UserPref userPref = ((OpenSocialData) data).getUserPrefs()
                                        .get(name);
                                if (userPref != null)
    ...
    private void registerHeaderColorChangement() {
        registerHandler(display.getHeaderColors()
                .addClickHandler(new ClickHandler() {
                    public void onClick(ClickEvent arg0) {
                        String color = ((ColorsPanelWidget) display.getHeaderColors()).getSelectedColor()
                                .getColorAsString();
    ...
    private void registerBorderColorChangement() {
        registerHandler(display.getBorderColors()
                .addClickHandler(new ClickHandler() {
                    public void onClick(ClickEvent arg0) {
                        String color = ((ColorsPanelWidget) display.getBorderColors()).getSelectedColor()
                                .getColorAsString();
    ...
    private void registerTitleColorChangement() {
        registerHandler(display.getTitleColors()
                .addClickHandler(new ClickHandler() {
                    public void onClick(ClickEvent arg0) {
                        String color = ((ColorsPanelWidget) display.getTitleColors()).getSelectedColor()
                                .getColorAsString();
