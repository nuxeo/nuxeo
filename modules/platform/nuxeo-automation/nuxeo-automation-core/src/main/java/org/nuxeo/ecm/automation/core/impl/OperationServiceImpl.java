...
public class OperationServiceImpl implements AutomationService, AutomationAdmin {
    ...
    public OperationType getOperation(String id) throws OperationNotFoundException {
        return (OperationType) operations.getContribution(
                id).orElseThrow(() -> new OperationNotFoundException("No operation was bound on ID: " + id));
    }
    ...
    public ChainException[] getChainExceptions() {
        return chainExceptions.getContributionValues().stream().toArray(ChainException[]::new);
    }
    ...
    public AutomationFilter[] getAutomationFilters() {
        return filters.getContributionValues().stream().toArray(AutomationFilter[]::new);
    }
