...
public class DirectoryRegistry extends MapRegistry {
    ...
    protected BaseDirectoryDescriptor getBareDirectoryDescriptor(String name) {
        DirectoryContributor contrib = this.<DirectoryContributor> getContribution(name).orElse(null);
        if (contrib == null) {
            return null;
        }
        MapRegistry targetRegistry = getTargetRegistry(contrib.target, contrib.point);
        return targetRegistry.<BaseDirectoryDescriptor> getContribution(name).orElse(null);
    }
