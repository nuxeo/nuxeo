...
public class MultiDirectorySession extends BaseSession {
    ...
    private void recomputeSourceInfos() {
        ...
        for (SourceDescriptor source : getDirectory().getDescriptor().sources) {
            ...
            for (SubDirectoryDescriptor subDir : source.subDirectories) {
                ...
                // record default field mappings if same name and record default values
                ...
                if (dirIsAuth) {
                    if (authDirectoryInfo != null) {
                        throw new DirectoryException(String.format(
                                "Directory '%s' source '%s' has two subdirectories with a password field: '%s' and '%s'",
            ...
            if (isAuthenticating() && authDirectoryInfo == null) {
                throw new DirectoryException(
                        String.format("Directory '%s' source '%s' has no subdirectory with a password field", getName(),
                                source.name));
            }
            if (!hasRequiredDir) {
                throw new DirectoryException(
        ...
        for (SourceInfo sourceInfo : sourceInfos) {
            ...
            for (SubDirectoryInfo dirInfo : sourceInfo.optionalSubDirectoryInfos) {
                ...
                for (DocumentModel entry : entries) {
                    ...
                    if (map != null) {
                        ...
                        log.warn(
                                "Entry '{}' for source '{}' is present in optional directory '{}' "
                                        + "but not in any required one: it will be skipped.",
            ...
            for (Entry<String, Map<String, Object>> e : maps.entrySet()) {
                ...
                if (seen.containsKey(id)) {
                    logDuplicateSource(id, seen.get(id), sourceInfo.source.name);
                    continue;
                }
                final Map<String, Object> map = e.getValue();
                if (counts.get(id) != numdirs) {
                    log.warn("Entry '{}' for source '{}' is not present in all directories: it will be skipped.", id,
            ...
            for (SubDirectoryInfo dirInfo : sourceInfo.subDirectoryInfos) {
                if (!dirInfo.getSession().isReadOnly()) {
                    ...
                    if (!sourceInfo.source.creation) {
                        ...
                        if (docModel == null) {
                            log.warn(
                                    "MultiDirectory '{}': the entry with id '{}' could not be deleted "
                                            + "on subdirectory '{}' because it does not exist",
                ...
                if (dirInfo.isOptional) {
                    ...
                    for (Map.Entry<String, Serializable> dirFilterEntry : dirFilter.entrySet()) {
                        ...
                        if ((defaultValue == null && filterValue != null)
                                || (defaultValue != null && !defaultValue.equals(filterValue))) {
                            matches = false;
                        }
                    }
                    if (matches) {
    ...
    public DocumentModelList query(QueryBuilder queryBuilder, boolean fetchReferences) {
        ...
        for (SourceInfo sourceInfo : sourceInfos) {
            ...
            for (String id : ids) {
                ...
                if (otherSource != null) {
                    logDuplicateSource(id, otherSource, sourceInfo.source.name);
                    continue;
                }
    ...
    protected void logDuplicateSource(String entryId, String source1, String source2) {
        log.warn("Entry '{}' is present in source '{}' but also in source '{}': the latter will be ignored.", entryId,
                source1, source2);
    }
