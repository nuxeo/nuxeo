...
public class TestDocUpdateWithDepotData {
    ...
    public void test() throws Exception {
        ...
        // assertEquals("we should have 12 files", 12, docs.size());
        assertEquals("we should have 10 files", 10, docs.size());

        DocumentRef ref = new PathRef("/Workspace-1/pv.pdf");
        DocumentModel doc = session.getDocument(ref);
        assertEquals("File pv.pdf should have 2 subjects", 2, ((String[]) doc.getPropertyValue("dc:subjects")).length);
        assertEquals("File pv.pdf should have 1 contributor", 1,
                ((String[]) doc.getPropertyValue("dc:contributors")).length);

        ref = new PathRef("/Workspace-1/odj.pdf");
        doc = session.getDocument(ref);
        assertNull("File odj.pdf should have 0 subjects", doc.getPropertyValue("dc:subjects"));
        assertEquals("File odj.pdf should have 2 Actors", 2,
                ((ArrayList<?>) doc.getPropertyValue("complx:Actors")).size());

        ref = new PathRef("/Workspace-1/pvcomplet.pdf");
        doc = session.getDocument(ref);
        assertEquals("File pvcomplet.pdf should have 5 subjects", 5,
                ((String[]) doc.getPropertyValue("dc:subjects")).length);
        assertEquals("File pvcomplet.pdf should have 3 contributors", 3,
                ((String[]) doc.getPropertyValue("dc:contributors")).length);

        ref = new PathRef("/Workspace-1/Acte-38/DECISION.pdf");
        doc = session.getDocument(ref);
        assertNull("File DECISION.pdf should have 0 subjects", doc.getPropertyValue("dc:subjects"));
        assertEquals("File DECISION.pdf should have 5 contributors", 5,
                ((String[]) doc.getPropertyValue("dc:contributors")).length);
        Boolean b = Arrays.asList((String[]) doc.getPropertyValue("dc:contributors")).contains("Waldo King");
        assertFalse("File DECISION.pdf should NOT contain a contributor named Waldo King", b);
        ...
        doc = session.getDocument(ref);
        assertNull("File odj.pdf should have 0 subjects", doc.getPropertyValue("dc:subjects"));
        assertEquals("File odj.pdf should have 3 Actors", 3,
                ((ArrayList<?>) doc.getPropertyValue("complx:Actors")).size());

        ref = new PathRef("/Workspace-1/pv.pdf");
        doc = session.getDocument(ref);
        assertEquals("File pv.pdf should have 2 subjects", 2, ((String[]) doc.getPropertyValue("dc:subjects")).length);
        assertEquals("File pv.pdf should have 2 contributors", 2,
                ((String[]) doc.getPropertyValue("dc:contributors")).length);

        ref = new PathRef("/Workspace-1/pvcomplet.pdf");
        doc = session.getDocument(ref);
        assertNull("File pvcomplet.pdf should have 0 subjects", doc.getPropertyValue("dc:subjects"));
        assertEquals("File pvcomplet.pdf should have 2 contributors", 2,
                ((String[]) doc.getPropertyValue("dc:contributors")).length);
        b = Arrays.asList((String[]) doc.getPropertyValue("dc:contributors")).contains("Eddie Murphy");
        assertTrue("File pvcomplet.pdf should contain a contributor named Eddie Murphy", b);
        b = Arrays.asList((String[]) doc.getPropertyValue("dc:contributors")).contains("Andres Blackwood");
        assertFalse("File pvcomplet.pdf should NOT contain a contributor named Andres Blackwood", b);

        ref = new PathRef("/Workspace-1/Acte-38/DECISION.pdf");
        doc = session.getDocument(ref);
        assertNull("File DECISION.pdf should have 0 subjects", doc.getPropertyValue("dc:subjects"));
        assertEquals("File DECISION.pdf should have 6 contributors", 6,
                ((String[]) doc.getPropertyValue("dc:contributors")).length);
        b = Arrays.asList((String[]) doc.getPropertyValue("dc:contributors")).contains("Waldo King");
        assertTrue("File DECISION.pdf should contain a contributor named Waldo King", b);

        docs = session.query("select * from Document where ecm:primaryType='Section'");
        assertEquals("we should have 1 Section", 1, docs.size());

        docs = session.query(
                "select * from Document where ecm:primaryType='Folder' AND ecm:parentId='" + seanceDoc.getId() + "'");
        assertEquals("we should have 3 actes in the seance", 3, docs.size());

        docs = session.query(
                "select * from Document where ecm:primaryType='Folder' AND ecm:parentId!='" + seanceDoc.getId() + "'");
        assertEquals("we should have only 1 actes outside of the seance", 1, docs.size());
