...
public class TestRegistry {
    ...
    public void testService() throws Exception {
        ...
        assertTrue(service.registryContributionRegistered);
        ...
        if (!useHotDeployer()) {
            // undeploy called
            assertTrue(service.registryContributionUnregistered);
        }
    }

    protected void checkSampleSingle(SingleRegistry reg, String name, String value) {
        Object obj = reg.getContribution();
        assertNotNull(obj);
        assertTrue(obj instanceof SampleSingleDescriptor);
        SampleSingleDescriptor desc = reg.getContribution(SampleSingleDescriptor.class);
    ...
    protected void checkSample(MapRegistry reg, String name, String value, Boolean bool) {
        Object obj = reg.getContribution(name);
        assertNotNull(obj);
        assertTrue(obj instanceof SampleDescriptor);
        SampleDescriptor desc = reg.getContribution(name, SampleDescriptor.class);
        assertEquals(name, desc.name);
        assertEquals(value, desc.value);
    ...
    public void testLegacyRegistry() throws Exception {
        assertNull(service.getLegacyRegistry());
        ...
        if (!useHotDeployer()) {
            // values have been cleared according to SampleComponent#unregister behavior (not hot reload management)
