...
//        title.setBorder("1px solid black");
    ...
    public void onClick(ClickEvent event) {        
        ...
        View<?> view = UI.getView("views/navigator");
        if (view instanceof NavigatorView) {
            NavigatorView nv = (NavigatorView)view;
            TreeGrid tree = nv.getWidget();
            TreeNode node = SmartClient.findNodeById(tree, parentId);
            if (node != null) {
                UI.openDocument(node.getAttribute("id"));
            } else {
                Window.alert("No Upper Node Found!");
            }
        }
    }
