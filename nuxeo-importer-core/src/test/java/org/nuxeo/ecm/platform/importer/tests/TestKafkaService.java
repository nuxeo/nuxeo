...
@LocalDeploy("org.nuxeo.ecm.platform.importer.core.test:test-kafka-service-contrib.xml")
public class TestKafkaService {

//    @Inject
//    private DefaultKafkaService mKafkaService;

    @Test
    public void testKafkaService() throws IOException {
        DefaultKafkaService kafkaService = Framework.getService(DefaultKafkaService.class);
        assertNotNull(kafkaService);
        assertEquals(1, kafkaService.allTopics().size());
    }
