...
public class ClusterLockHelper {
    ...
    private static final Log log = LogFactory.getLog(ClusterLockHelper.class);
    ...
    protected String tryLock(String key) {
        ...
        do {
            ...
            if (kvStore.compareAndSet(key, null, lockInfo, ttl)) {
                // lock acquired
                log.debug("Lock '" + key + "' acquired after "
                        + (System.nanoTime() - (deadline - duration.toNanos())) / 1_000_000 + "ms");
                return lockInfo;
            }
            // wait a bit before retrying
            log.debug("  Sleeping on busy lock '" + key + "' for " + pollDelay.toMillis() + "ms");
            try {
        ...
        log.debug("Failed to acquire lock '" + key + "' after " + duration.getSeconds() + "s");
        return null;
    }
    ...
    protected void unLock(String key, String lockInfo) {
        ...
        if (current == null) {
            ...
            log.warn("Unlocking '" + key + "' but the lock had already expired; "
                    + "consider increasing the try duration for this lock");
        } else {
            // lock expired and was reacquired
            log.error("Failed to unlock '" + key + "', the lock expired and has a new owner: " + getLock(key) + "; "
                    + "consider increasing the try duration for this lock");
        }
    }
