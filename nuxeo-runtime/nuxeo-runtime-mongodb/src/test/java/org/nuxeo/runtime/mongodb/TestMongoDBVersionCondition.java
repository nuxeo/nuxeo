...
public class TestMongoDBVersionCondition {
    ...
    public void testCountCall() {
        MongoDatabase database = Framework.getService(MongoDBConnectionService.class).getDatabase(DEFAULT_DATABASE_ID);
        ...
        assertEquals(3, countDocuments(DEFAULT_DATABASE_ID, coll));
        assertEquals(2, countDocuments(DEFAULT_DATABASE_ID, coll, filter));
        assertEquals(1, countDocuments(DEFAULT_DATABASE_ID, coll, filter, options));
    }
}
