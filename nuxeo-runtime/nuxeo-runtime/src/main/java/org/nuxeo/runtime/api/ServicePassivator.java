...
import org.nuxeo.runtime.api.LambdaAdaptor.RunnableCheckException;
import org.nuxeo.runtime.api.LambdaAdaptor.Supplier;
...
/**
 * Blocks service accesses in order to run an operation which alter the runtime.
 * That gives a way to prevent service consumers to enter during the shutdown or
...
public class ServicePassivator {
	...
	public static Termination proceed(long quiet, long timeout, boolean enforce, Runnable runnable) {
		return passivate().withQuietDelay(quiet).monitor().withTimeout(timeout).withEnforceMode(enforce).await()
				.proceed(runnable);
	}


	public static <X extends Exception> void proceed(long quiet, long timeout, boolean enforce,
			final RunnableCheckException<X> runnable, Class<X> oftype) throws X {
		...
		try {
			ServicePassivator.passivate().withQuietDelay(quiet).monitor().withTimeout(timeout).withEnforceMode(enforce)
					.await().proceed(new Runnable() {
	...
	public static class Passivator {
		...
		void run() {
			...
			ServiceProvider passthrough = installed.map(new Function<ServiceProvider, ServiceProvider>() {
				public ServiceProvider apply(ServiceProvider t) {
					return new DelegateProvider(t);
				}
			}).orElseGet(new Supplier<ServiceProvider>() {
		...
		void resetProvider() {
			...
			ServiceProvider previous = passivator.passthrough instanceof DelegateProvider
					? ((DelegateProvider) passivator.passthrough).next : null;
		...
		public class Accounting {
			...
			Optional<InScopeOfContext> take(Class<?> serviceof) {
				final Class<?>[] callstack = dumper.dump();
				Optional<InScopeOfContext> snapshot = inscopeof(callstack)
						.map(new LambdaAdaptor.Function<Class<?>, InScopeOfContext>() {
				...
				snapshot.ifPresent(new Consumer<InScopeOfContext>() {
			...
			Optional<Class<?>> inscopeof(Class<?>[] callstack) {
				final ComponentManager cm = Framework.getRuntime().getComponentManager();
				if (cm != null) {

					for (Class<?> typeof : callstack) {
						if (cm.getComponentProvidingService(typeof) != null) {
							return (Optional) Optional.of(typeof);
						}
					}
				}
			...
			public class InScopeOfContext {
				...
				public String toString() {
					StringBuilder builder = new StringBuilder().append("on ").append(thread).append(" in scope of ")
							.append(serviceof).append(System.lineSeparator());
					for (Class<?> typeof : callstack) {
	...
	public static class Monitor {
		...
		void run() {
			long delay = TimeUnit.MILLISECONDS.convert(quietDelay, TimeUnit.SECONDS);
			if (delay <= 0) {
				passivated.countDown();
				return;
			}
		...
		/**
		 * Installs the timer task which monitor the service lookups. Once there
		 * will be no more lookups in the scheduled period, notifies the runner
		 * for proceeding.
		 *
		 * @param next
		 * @return
		...
		public Termination proceed(Runnable runnable) {
			try {
				...
				return monitor.passivator.accounting.last
						.<Termination>map(new Function<InScopeOfContext, Termination>() {

							@Override
							public Termination apply(InScopeOfContext context) {
								return new Termination.Failure(context);
							}
						}).orElseGet(new Supplier<Termination>() {
	...
	/**
	 * Terminates the pacification by a success or a failure action and release
	 * the lock.
	 */
	public interface Termination {
		 ...
		 * @param runnable
		 */
		Termination onSuccess(Runnable finisher);

		/**
		 * Recover the failure if the passivation was a failure, ie: some
		 * activity has been detected during the protected operation.
		 *
		 * @param runnable
		 *            the failure action
		 */
		Termination onFailure(Consumer<InScopeOfContext> recoverer);

		Termination peek(Consumer<Termination> consumer);
		...
		class Success implements Termination {
			...
			public Termination peek(Consumer<Termination> consumer) {
				consumer.accept(this);
				return this;
			}
	...
	static class PassivateProvider implements ServiceProvider {
		...
		public <T> T getService(Class<T> typeof) {
			...
			return accounting.take(typeof).map(new Function<InScopeOfContext, ServiceProvider>() {

				@Override
				public ServiceProvider apply(InScopeOfContext value) {
					return passthrough;
				}
			}).orElse(waitfor).getService(typeof);
		}
	}
