...
public class PermissionListener implements EventListener {
    ...
    public static final String ACE_INFO_DIRECTORY = "aceinfo";
    ...
    public void handleEvent(Event event) throws ClientException {
        ...
        if (DOCUMENT_SECURITY_UPDATED.equals(event.getName())) {
            ...
            ACP oldACP = (ACP) ctx.getProperty(OLD_ACP);
            ACP newACP = (ACP) ctx.getProperty(NEW_ACP);
            updateDirectory(doc, oldACP, newACP);
        }
    }

    protected void updateDirectory(DocumentModel doc, ACP oldACP, ACP newACP) {
        ...
        for (ACLDiff diff : aclDiffs) {
            ...
            try {
                ...
                for (ACE ace : diff.addedACEs) {
                    ...
                    m.put("aceinfo:comment", ace.getContextData(COMMENT_KEY));
