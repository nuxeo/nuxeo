...
public class TestFormData {
    ...
    protected static final Map<String, String[]> REQUEST_MAP = new HashMap<>();
    static {
        REQUEST_MAP.put("dc:title", new String[] { "My title" });
        REQUEST_MAP.put("dc:description", new String[] { "My description" });
    }
    ...
    protected void checkFormFields(FormData formData) {
        ...
        for (String key : REQUEST_MAP.keySet()) {
            ...
            assertEquals(Arrays.asList(REQUEST_MAP.get(key)), Arrays.asList(fields.get(key)));
        }
    }
