...
public class QuotaComputerProcessor implements PostCommitEventListener {
            ...
            if (DOCUMENT_CHECKEDIN.equals(sourceEvent)) {
                ...
                if (quotaDoc.getInnerSize() > 0) {
                    processOnParents(parents, quotaCtx.getBlobSize() + quotaCtx.getVersionsSize(), 
                            quotaCtx.getBlobSize() + quotaCtx.getVersionsSize(),
    ...
    protected void processOnParents(List<DocumentModel> parents, long delta, long trash)
            throws ClientException {
        processOnParents(parents, delta, trash, 0L, true, true, false);
    }

    protected void processOnParents(List<DocumentModel> parents, long delta, long trash, long deltaVersions)
            throws ClientException {
        processOnParents(parents, delta, trash, deltaVersions, true, true, false);
    }
