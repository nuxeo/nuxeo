...
var searchreplace = (function () {
  ...
  var $_6x26w6isje4cbxfb = { findAndReplaceDOMText: findAndReplaceDOMText };
  ...
  var markAllMatches = function (editor, currentIndexState, regex) {
    ...
    return $_6x26w6isje4cbxfb.findAndReplaceDOMText(regex, node, marker, false, editor.schema);
  };
  ...
  var get = function (editor, currentIndexState) {
    var done = function (keepEditorSelection) {
      return $_eocwqbiqje4cbxf5.done(editor, currentIndexState, keepEditorSelection);
    };
    var find = function (text, matchCase, wholeWord) {
      return $_eocwqbiqje4cbxf5.find(editor, currentIndexState, text, matchCase, wholeWord);
    };
    var next = function () {
      return $_eocwqbiqje4cbxf5.next(editor, currentIndexState);
    };
    var prev = function () {
      return $_eocwqbiqje4cbxf5.prev(editor, currentIndexState);
    };
    var replace = function (text, forward, all) {
      return $_eocwqbiqje4cbxf5.replace(editor, currentIndexState, text, forward, all);
    };
    return {
      done: done,
      find: find,
  ...
  var open = function (editor, currentIndexState) {
    ...
    function updateButtonStates() {
      win.statusbar.find('#next').disabled($_eocwqbiqje4cbxf5.hasNext(editor, currentIndexState) === false);
      win.statusbar.find('#prev').disabled($_eocwqbiqje4cbxf5.hasPrev(editor, currentIndexState) === false);
    }
    ...
    var win = editor.windowManager.open({
      ...
      onClose: function () {
        editor.focus();
        $_eocwqbiqje4cbxf5.done(editor, currentIndexState);
        editor.undoManager.add();
      },
      onSubmit: function (e) {
        ...
        if (!text.length) {
          $_eocwqbiqje4cbxf5.done(editor, currentIndexState, false);
          win.statusbar.items().slice(1).disabled(true);
          return;
        }
        if (last.text === text && last.caseState === caseState && last.wholeWord === wholeWord) {
          if (!$_eocwqbiqje4cbxf5.hasNext(editor, currentIndexState)) {
            notFoundAlert();
            return;
          }
          $_eocwqbiqje4cbxf5.next(editor, currentIndexState);
          updateButtonStates();
          return;
        }
        count = $_eocwqbiqje4cbxf5.find(editor, currentIndexState, text, caseState, wholeWord);
        if (!count) {
          notFoundAlert();
        }
      ...
      buttons: [
        ...
        },
        {
          ...
          disabled: true,
          onclick: function () {
            if (!$_eocwqbiqje4cbxf5.replace(editor, currentIndexState, win.find('#replace').value())) {
              win.statusbar.items().slice(1).disabled(true);
              currentIndexState.set(-1);
              last = {};
            }
        ...
        },
        {
          ...
          disabled: true,
          onclick: function () {
            $_eocwqbiqje4cbxf5.replace(editor, currentIndexState, win.find('#replace').value(), true, true);
            win.statusbar.items().slice(1).disabled(true);
            last = {};
          }
        },
        ...
        },
        {
          ...
          disabled: true,
          onclick: function () {
            $_eocwqbiqje4cbxf5.prev(editor, currentIndexState);
            updateButtonStates();
          }
        },
        {
  ...
  var $_4fyx4wiuje4cbxfi = { open: open };

  var register = function (editor, currentIndexState) {
    editor.addCommand('SearchReplace', function () {
      $_4fyx4wiuje4cbxfi.open(editor, currentIndexState);
    });
  };
  var $_3yu4t8itje4cbxfh = { register: register };
  ...
  var $_4799a2ivje4cbxfs = { register: register$1 };

  PluginManager.add('searchreplace', function (editor) {
    var currentIndexState = Cell(-1);
    $_3yu4t8itje4cbxfh.register(editor, currentIndexState);
    $_4799a2ivje4cbxfs.register(editor, currentIndexState);
    return $_d01je5ipje4cbxf3.get(editor, currentIndexState);
  });
  function Plugin () {
  }
