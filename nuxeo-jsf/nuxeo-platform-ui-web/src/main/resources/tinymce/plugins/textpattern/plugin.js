...
var textpattern = (function () {
  ...
  var $_efy8xqqhje4cbz9n = { getPatterns: getPatterns };
  ...
  var patternFromRng = function (patterns, rng, space) {
    ...
    var endPattern = $_9b7n5zqpje4cbz9x.findEndPattern(patterns, text, rng.startOffset, delta);
    ...
    var startPattern = $_9b7n5zqpje4cbz9x.findPattern(patterns, patternRng.toString());
  ...
  var applyBlockFormat = function (editor, patterns) {
    ...
    if (textBlockElm) {
      ...
      if (firstTextNode) {
        pattern = $_9b7n5zqpje4cbz9x.findPattern(patterns, firstTextNode.data);
        if (!pattern) {
          return;
        }
  ...
  function handleEnter(editor, patterns) {
    ...
    wrappedTextNode = $_1705njqmje4cbz9t.applyInlineFormatEnter(editor, patterns);
    if (wrappedTextNode) {
    ...
    $_1705njqmje4cbz9t.applyBlockFormat(editor, patterns);
  }
  function handleInlineKey(editor, patterns) {
    var wrappedTextNode, lastChar, lastCharNode, rng, dom;
    wrappedTextNode = $_1705njqmje4cbz9t.applyInlineFormatSpace(editor, patterns);
    if (wrappedTextNode) {
      dom = editor.dom;
  ...
  var setup = function (editor, patternsState) {
    ...
    editor.on('keydown', function (e) {
      if (e.keyCode === 13 && !VK.modifierPressed(e)) {
        $_52v64gqlje4cbz9q.handleEnter(editor, patternsState.get());
      }
    }, true);
    editor.on('keyup', function (e) {
      if ($_52v64gqlje4cbz9q.checkKeyCode(keyCodes, e)) {
        $_52v64gqlje4cbz9q.handleInlineKey(editor, patternsState.get());
      }
    });
    editor.on('keypress', function (e) {
      if ($_52v64gqlje4cbz9q.checkCharCode(charCodes, e)) {
  ...
  var $_db02ruqije4cbz9o = { setup: setup };

  PluginManager.add('textpattern', function (editor) {
    var patternsState = Cell($_efy8xqqhje4cbz9n.getPatterns(editor.settings));
    $_db02ruqije4cbz9o.setup(editor, patternsState);
    return $_a0vhiwqgje4cbz9m.get(patternsState);
  });
  function Plugin () {
  }
