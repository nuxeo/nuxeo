...
public class TestMongoDBRepositoryQuery extends MongoDBRepositoryTestCase {
    ...
    protected void initRepository() throws Exception {
        ...
        deployContrib("org.nuxeo.ecm.core.storage.mongodb.tests", "OSGI-INF/test-repo-types-query.xml");
        deployContrib("org.nuxeo.ecm.core.storage.mongodb.tests", "OSGI-INF/test-repo-types-2.xml");
    ...
    public void testQueryMixinTypeNotPerDocument() throws Exception {
        ...
        // don't test on NotPerDocFacet which is only defined as no-per-instance in VCS repo config
        // doc has facet but not found by search
        DocumentModelList dml = session.query("SELECT * FROM Document WHERE ecm:mixinType = 'NotPerDocFacet2'");
        assertEquals(0, dml.size());
    }
    ...
    public void testQueryIterableWithTransformer() throws Exception {
        ...
        deployContrib("org.nuxeo.ecm.core.storage.mongodb.tests", "OSGI-INF/security-policy2-contrib.xml");
    ...
    public void testQueryComplexOrderBy() throws Exception {
        ...
        try {
            session.query(SELECT_WHERE + clause);
            // fail(); // ORDER BY tst:subjects works on MongoDB
        } catch (QueryParseException e) {
