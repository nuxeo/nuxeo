...
public class OrphanVersionCleanupListener implements PostCommitEventListener {
    ...
    /**
     * Framework property for the maximum number of orphan versions to delete in one transaction. Default is
    ...
    protected long getCommitSize() {
        String commitSize = Framework.getProperty(DEFAULT_COMMIT_SIZE_PROP);
        if (commitSize == null) {
        ...
        try {
            ...
            log.error("Invalid property " + DEFAULT_COMMIT_SIZE_PROP, e);
            return DEFAULT_COMMIT_SIZE;
        }
    }
