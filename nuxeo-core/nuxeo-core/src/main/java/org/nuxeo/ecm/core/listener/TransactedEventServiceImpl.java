...
public class TransactedEventServiceImpl implements TransactedEventService {
	...
	private final static Log log = LogFactory.getLog(TransactedEventServiceImpl.class);
	
	protected static ThreadLocal<EventList> events = new ThreadLocal<EventList>() {
		protected EventList initialValue() { return new EventList(false); }
	};
	...
	public void addListener(TransactedListener listener) {
	    if (listener instanceof OnSaveListener) {
	        ...
	        throw new UnsupportedOperationException("TransactionListener of type '"+listener.getClass().getName()+"' is not known");
	    }
	}
	...
	public void recordEvent(CoreEvent event) {
		...
		if (list.operations != null) {
			if (log.isDebugEnabled()) {
				log.debug("Ingoring post commit recording for core event "+event.getEventId()+" since there is an operation context");
			}
			return; 
		}
