...
@Deploy({ "org.nuxeo.ecm.core.mongodb.test:OSGI-INF/mongodb-disable-renaming-listener-test.xml",
        "org.nuxeo.ecm.core.mongodb.test:OSGI-INF/mongodb-enable-unique-indices-contrib.xml" })
public class TestMongoDBIndices {
    ...
    public void shouldFailWhenCreatingExistingChildNameDocument() {
        try (CoreSession bobSession = CoreInstance.openCoreSession(coreSession.getRepositoryName(), "bob")) {
            createDocument(bobSession, folder, DOCUMENT_NAME);
        }

        try (CoreSession jamesSession = CoreInstance.openCoreSession(coreSession.getRepositoryName(), "james")) {
