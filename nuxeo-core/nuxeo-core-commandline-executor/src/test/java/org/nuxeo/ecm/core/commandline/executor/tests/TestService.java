...
package org.nuxeo.ecm.core.commandline.executor.tests;
...
public class TestService extends NXRuntimeTestCase {
    ...
    public void testCmdRegistration() throws Exception {
        ...
        deployContrib("org.nuxeo.ecm.core.commandline.executor", "OSGI-INF/commandline-aspell-test-contribs.xml");
        List<String> cmds = cles.getRegistredCommands();
        assertNotNull(cmds);
        assertEquals(1, cmds.size());
        assertTrue(cmds.contains("aspell"));

        deployContrib("org.nuxeo.ecm.core.commandline.executor", "OSGI-INF/commandline-imagemagic-test-contrib.xml");
        cmds = cles.getRegistredCommands();
        assertNotNull(cmds);
        assertEquals(2, cmds.size());
        assertTrue(cmds.contains("identify"));

        deployContrib("org.nuxeo.ecm.core.commandline.executor",
    ...
    public void testCmdAvailable() {
         ...
         * assertTrue(cmds.contains("aspell")); deployContrib("org.nuxeo.ecm.core.commandline.executor",
         ...
         * deployContrib("org.nuxeo.ecm.core.commandline.executor","OSGI-INF/commandline-dummy-test-contrib.xml");
