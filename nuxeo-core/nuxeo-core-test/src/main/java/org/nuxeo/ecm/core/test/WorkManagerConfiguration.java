...
public class WorkManagerConfiguration {
    ...
    private static final String STREAM_WORK_MANAGER_STORESTATE_ENABLED_PROPERTY =
            "nuxeo.test.stream.workmanager.storestate.enabled";
    ...
    protected void init() {
        ...
        streamWorkManagerStoreStateEnabled = Boolean.parseBoolean(
    ...
    protected String calcWorkManagerType() {
        ...
        if (StringUtils.isNotEmpty(value)) {
            if (Stream.of(WORK_MANAGER_DEFAULT, WORK_MANAGER_CHRONICLE_STREAM, WORK_MANAGER_KAFKA_STREAM).noneMatch(
                    s -> s.equals(value))) {
                throw new NuxeoException(String.format(
                        "Optional system property '%s' has illegal value '%s'", WORK_MANAGER_PROPERTY, value));
            }
            return value;
        };
    ...
    public List<URL> getDeploymentContribURLs(FeaturesRunner runner) {
        ...
        default:
            throw new NuxeoException(String.format(
                    "WorkManagerConfiguration.workManagerType '%s' is invalid, check system property %s",
                    workManagerType, WORK_MANAGER_PROPERTY));
        }
        return contribURLs;
    }
    ...
    private URL getDeploymentContribURL(FeaturesRunner runner, String bundleName, String contribPath) {
        Bundle bundle = runner.getFeature(
                RuntimeFeature.class).getHarness().getOSGiAdapter().getRegistry().getBundle(bundleName);
