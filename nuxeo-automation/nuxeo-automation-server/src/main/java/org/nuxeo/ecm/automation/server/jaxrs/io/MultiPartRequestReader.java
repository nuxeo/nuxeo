...
public class MultiPartRequestReader implements MessageBodyReader<ExecutionRequest> {
    ...
    public ExecutionRequest readFrom(Class<ExecutionRequest> arg0, Type arg1,
        ...
        try {
            ...
            // javax.mail fail to receive some parts - I am not sure why - perhaps the stream is no more available when javax.mail need it?
            ...
            try {
                ...
                if (cnt == 2) { // a blob
                    ...
                    for (int i=0 ; i<cnt; i++) {
                        log.error("Received parts: "+mp.getBodyPart(i).getHeader("Content-ID")[0]+" -> "+mp.getBodyPart(i).getContentType());
                    }
    ...
    public static Blob readBlob(HttpServletRequest request, BodyPart part) throws Exception {
        ...
        UserSession.addRequestCleanupHandler(request, new RequestCleanupHandler() {
