# Nuxeo LTS 2023 - Agent Development Guide

## Project Overview

Nuxeo is an Enterprise Content Management (ECM) platform. This is the `nuxeo-lts` monorepo (version `2023.x-SNAPSHOT`), a massive Maven multi-module Java project hosted at `github.com/nuxeo/nuxeo-lts` on branch `2023`.
...
nuxeo-ecm (root)
├── modules/
│   ├── runtime/             # ~25 modules - Core runtime, OSGi, streams, Kafka, MongoDB driver, KV store
│   ├── core/                # ~26 modules - Document model, storage engines (SQL, MongoDB, DBS, mem), bulk ops, binary managers
│   └── platform/            # ~98 modules - High-level services: REST API, audit, auth, automation, workflows, Drive, CMIS
...
├── packages/                # ~35 Nuxeo marketplace packages
├── ftests/                  # ~35 functional test modules (feature-based organization)
...
```
nuxeo-ecm (root, org.nuxeo)             # All dependency versions managed here (~5900 lines)
...
- **Maven** multi-module with root POM managing all dependency versions (~5900 lines)
- **Java 17** compiler target (`<release>17</release>`) with **Azul Zulu JDK 21** runtime
- **Maven 3.6.3+** required
- Default build: `mvn install` (builds `modules` only)
- Full build: `mvn install -Pdistrib,docker -DskipTests -T6`
- Code formatting: `mvn spotless:check` / `mvn spotless:apply` (Eclipse formatter, ratcheted from `origin/2023`)
...
The compiler target is Java 17. Use all Java 17 features:
...
- **`@since`** tag is mandatory on all new public API (classes, methods, fields):
  - Current format: `@since 2023.XX` (CalVer: LTS year + sprint number)
  - Example: `@since 2023.17`
- Inline `// @since 2023.XX` comments are acceptable on fields and private/protected members
...
1. Static imports
2. `java.*`
3. `javax.*` (`javax.inject`, `javax.servlet`, `javax.ws.rs`, etc.)
4. `org.*` (`org.apache.*`, `org.nuxeo.*`, etc.)
5. `com.*` (`com.google.*`, `com.fasterxml.*`, etc.)

Note: The project uses the **`javax.*`** namespace (`javax.inject`, `javax.servlet`, `javax.ws.rs`), NOT `jakarta.*`. Jakarta namespace artifacts are **banned** by the enforcer plugin.

## Backward Compatibility

- **Never break backward compatibility** unless explicitly asked to
- When adding new methods to replace old ones:
  1. Add `@Deprecated(since = "2023.XX", forRemoval = true)` to the old method
  2. Add `/** @deprecated since 2023.XX, use {@link NewMethod} instead */` Javadoc
  3. Update all internal callers to use the new method (the codebase must not use deprecated methods)

### Deprecation Format

```java
/** @deprecated since 2023.17, use {@link #newMethod()} instead */
@Deprecated(since = "2023.17", forRemoval = true)
...
- Built on **Nuxeo WebEngine** (custom framework on top of Jersey 1.x / JAX-RS 1.1)
- Uses `javax.ws.rs` annotations (`@GET`, `@POST`, `@Path`, `@Produces`)
...
- **`@Inject`**: `javax.inject` for services (NOT Spring `@Autowired`)
...
- `jakarta.*` artifacts (use `javax.*` equivalents -- this codebase has NOT migrated to Jakarta EE)
- Log4j 1.x / Logback directly (use Log4j2)
- Old ASM versions (`asm:asm` -- use `org.ow2.asm:asm`)
- Old Bouncy Castle JDK15 artifacts (use `*-jdk18on`)
- Various duplicate servlet-api artifacts (use `javax.servlet:javax.servlet-api`)
...
- Release commits: `Release 2023.XX, update 2023.XX-SNAPSHOT to 2023.YY-SNAPSHOT`
...
- `task-`: Tasks/chores
- Append `-2023` suffix when targeting the 2023 LTS branch
...
2. **Do NOT use `jakarta.*` imports** - the project uses `javax.*` namespace; Jakarta artifacts are banned by the enforcer plugin
