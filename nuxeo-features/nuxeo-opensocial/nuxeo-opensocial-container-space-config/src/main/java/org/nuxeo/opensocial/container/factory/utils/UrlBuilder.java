...
public class UrlBuilder {
	...
	public static String buildShindigUrl(GadgetMapper gadget) throws Exception {
		...
		return ServerBase.getBase() + SERVLET_PATH + "?" + getDefaultParams()
				+ "&" + GADGET_ID_KEY + "=" + gadget.getShindigId() + "&"
				+ PARENT_KEY + "=" + ServerBase.getBase() + "&"
				+ PERMISSION_KEY + "=" + gadget.getPermission() + "&" + URL_KEY
				+ "=" + gadgetDef + getUserPrefs(gadget.getUserPrefs()) + "&"
				+ SECURITY_TOKEN_KEY + "="
	...
	private static String getSecurityToken(GadgetMapper gadget, String url)
			throws Exception {
		return SecureTokenBuilder.getSecureToken(gadget.getViewer(), gadget
				.getOwner(), url);
	}
