...
public class CoreOperationsTest {
    ...
    @Test public void testScriptOperation() throws Exception {
     ...
     * Enable this test when the operation will be enabled.
     */
    @Ignore
    @Test public void testScriptFileOperation() throws Exception {
        OperationContext ctx = new OperationContext(session);
        ctx.setInput(src);
    ...
    /**
     * Same as before but tests relative paths
     * Create | Copy | Set Property
     * This is also testing {@link StringToProperties} adapter
     * @throws Exception
     */
    @Test public void testChain1WithRelativePath() throws Exception {
        OperationContext ctx = new OperationContext(session);
        ctx.setInput(src);
     ...
     * Same as before
     * Create | Copy | Set Property
     * But also test properties specified using a mvel Expression
     * @throws Exception
     */
    @Test public void testMvelExpressionProperties() throws Exception {
     ...
     * @throws Exception
     */
    @Test public void testMvelExpressionProperties2() throws Exception {
        src.setPropertyValue("dc:description", "dc:title=MyDoc");
        session.saveDocument(src);
        OperationContext ctx = new OperationContext(session);
        ctx.setInput(src);
    ...
    @Test public void testChain3() throws Exception {
        ...
        OperationChain chain = new OperationChain("testChain");
        chain.add(FetchContextDocument.ID);
        chain.add(CreateDocument.ID).set("type", "Note").
            set("properties", new Properties("dc:title=MyDoc")).set("name", "note");
    ...
    @Test public void testChain4() throws Exception {
        ...
        OperationChain chain = new OperationChain("testChain");
        chain.add(FetchContextDocument.ID);
        chain.add(CreateDocument.ID).set("type", "Note").set("name", "note1")
