...
public class DocumentHelper {
    ...
    /**
     * Save the document and clear context data to avoid incrementing version in next operations if not needed.
     * @param doc
     * @return
     */
    ...
    /**
     * Remove a property from a document given the xpath. If the xpath points to a list property the list will be cleared.
     * If the path points to a blob in a list the property is removed from the list.
     * Otherwise the xpath should point to a non list property that will be removed.
     * @param doc
     * @param xpath
    ...
    /**
     * Given a document property update its value with the given blob.
     * The property can be a blob list or a blob.
     * If a blob list the blob is appended to the list, if a blob then it will be set as the property value.
     * Both blob list formats are supported: the file list (blob holder list) and simple blob list.
     * @param p
     * @param blob
     */
    public static void addBlob(Property p, Blob blob) throws PropertyException {
        if (p.isList()) {
            // detect if a list of simple blobs or a list of files (blob holder)
            ...
            if (ft.isComplexType() && ((ComplexType)ft).getFieldsCount() == 2) {
                ...
                p.add((Serializable)blob);
            }
        } else {
            p.setValue((Serializable)blob);
        }
    }
     ...
     * There is one special property: ecm:acl that can be used to set the local acl.
     * The format of this property value is:
     * [string username]:[string permission]:[boolean grant], [string username]:[string permission]:[boolean grant], ...
     *
     * TODO list properties are not yet supported
     *
     * @param doc
     * @param values
     * @throws Exception
