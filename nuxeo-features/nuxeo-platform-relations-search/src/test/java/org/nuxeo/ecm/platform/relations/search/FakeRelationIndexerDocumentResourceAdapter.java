...
public class FakeRelationIndexerDocumentResourceAdapter
    ...
    @Override
    public QNameResource getResource(Object object) {
        DocumentModel dm = (DocumentModel) object;
        if (!namespace.endsWith("docfam/")) {
            return new QNameResourceImpl(namespace, dm.getId());
        }
        return new QNameResourceImpl(namespace, dm.getId().split("_")[0]);
    }
