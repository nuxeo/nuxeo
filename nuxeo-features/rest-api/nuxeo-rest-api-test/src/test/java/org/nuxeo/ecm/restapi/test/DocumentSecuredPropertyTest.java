...
public class DocumentSecuredPropertyTest extends BaseTest {
    ...
    public void testAdministratorCanCreate() {
        ClientResponse response = getResponse(RequestType.POST, "path/folder_2",
    ...
    public void testUserCanUseRepositoryUsingEmptyWithDefaultAdapter() throws IOException {
        this.service = getServiceFor(USER_1, USER_1);
        String docModel;
        ClientResponse response = getResponse(RequestType.GET, "path/folder_2/@emptyWithDefault",
        ...
        try {
            ...
            ObjectNode unsecureComplex = root.node.with("properties").with("secured:unsecureComplex");
            unsecureComplex.put("scalar2", "I can");
            root.node.with("properties").put("secured:unsecureComplex", unsecureComplex);
            docModel = root.asJson();
        } finally {
            response.close();
        }
        response = getResponse(RequestType.POST, "path/folder_2", docModel);
        try {
            StatusType status = response.getStatusInfo();
            assertEquals("HTTP Reason: " + status.getReasonPhrase(), SC_CREATED, status.getStatusCode());
        } finally {
        ...
        ObjectNode properties = root.node.with("properties");
        ObjectNode unsecureComplex = properties.with("secured:unsecureComplex");
        properties.removeAll();
        unsecureComplex.put("scalar2", "I still can!");
        properties.put("secured:unsecureComplex", unsecureComplex);
        docModel = root.asJson();
        response = getResponse(RequestType.POST, "path/folder_2", docModel);
        try {
            StatusType status = response.getStatusInfo();
