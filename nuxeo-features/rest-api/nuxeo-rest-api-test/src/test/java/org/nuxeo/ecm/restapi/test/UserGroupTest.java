...
public class UserGroupTest extends BaseUserTest {
	...
	public void itCanSearchGroups() throws Exception {
		...
		ArrayNode entries = (ArrayNode) node.get("entries");
		assertEquals(1, entries.size());
		assertEquals("Lannister", entries.get(0).get("grouplabel")
				.getValueAsText());
	...
	public void itDoesntWritePassword() throws Exception {
		...
		assertEquals("", node.get("properties").get("password")
				.getValueAsText());
	 ...
	 *            node to test
	 * @param strings
	 *            an array of expected user names
	 *
	 * @since 5.8
	 */
	...
	private void assertPaging(int currentPageIndex, int pageSize,
		...
		assertEquals(currentPageIndex, node.get("currentPageIndex")
				.getIntValue());
	 ...
	 *            node to test
	 * @param strings
	 *            an array of expected group names
	 *
	 * @since 5.8
	 */
	private void assertGroupEntries(JsonNode node, String... groups) {
		...
		for (int i = 0; i < groups.length; i++) {
			assertEquals(groups[i], entries.get(i).get("groupname")
					.getValueAsText());
		}
	}
