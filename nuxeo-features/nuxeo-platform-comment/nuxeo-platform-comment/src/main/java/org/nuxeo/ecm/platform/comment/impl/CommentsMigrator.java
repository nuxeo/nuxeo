...
public class CommentsMigrator extends AbstractRepositoryMigrator {
    ...
    protected void migrateCommentsFromPropertyToSecured(CoreSession session, CommentManager commentManager,
                                                        IdRef commentIdRef) {
        // Move the commentIdRef under the commented document
        session.move(commentIdRef, commentManager.getAncestorRef(session, commentIdRef), null);
    ...
    protected List<String> getUnsecuredCommentIds(CoreSession session) {
        ...
        String query = String.format("SELECT %s FROM Comment WHERE %s IN (%s)", ECM_UUID, ECM_PARENTID, buildInClause(parentIds));
