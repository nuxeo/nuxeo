    ...
    protected void updateCommentProperties(CoreSession session, String ecmParentId, String commentParentId,
        ...
        do {
            ...
            for (DocumentModel docModel : docModels) {
                ...
                ArrayList<String> commentAncestorIds = new ArrayList<>(Arrays.asList(parentCommentAncestorIds));
                commentAncestorIds.add(commentParentId);
                docModel.setPropertyValue(COMMENT_ANCESTOR_IDS, commentAncestorIds);
                docModel.setPropertyValue(COMMENT_PARENT_ID, commentParentId);
                session.saveDocument(docModel);
                // loop on replies
                updateCommentProperties(session, docModel.getId(), docModel.getId(),
                        commentAncestorIds.toArray(new String[0]));
            }
            offset += limit;
            if (total == 0) {
